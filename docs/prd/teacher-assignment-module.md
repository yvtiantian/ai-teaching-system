...
当作业仅包含 `single_choice`、`multiple_choice`、`true_false` 时，学生提交后 `student_submit` 自动完成批改：
- 客观题 SQL 精确匹配评分
- 提交状态直接设为 `graded`（跳过 AI 批改和教师复核）
- 教师无需操作即可看到最终成绩

当作业包含 `fill_blank` 但不包含需要 AI 处理的 `short_answer` 时：
- 系统会先完成自动评分
- 提交状态进入 `auto_graded`
- 学生端显示为“已判分”
- 教师端保留“可复核”入口，但这不是必须经过的流程

当作业包含有有效作答内容的 `short_answer` 时：
- 提交状态进入 `submitted` → `ai_grading` → `ai_graded`
- 教师复核属于必经流程
